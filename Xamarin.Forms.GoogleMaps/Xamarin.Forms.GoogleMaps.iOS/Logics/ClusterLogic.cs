...
namespace Xamarin.Forms.GoogleMaps.Logics.iOS
{
    internal class ClusterLogic : DefaultPinLogic<ClusteredMarker, MapView>
    {
        ...
        protected IList<Pin> GetNonClusteredItems(Map map) => map.Pins;
        ...
        {
            if (pin != null)
                NativeMap.SelectedMarker = (Google.Maps.Marker)pin.NativeObject;
            else
        ...
        {
            return GetItems(Map).FirstOrDefault(outerItem => ((Marker)outerItem.NativeObject).Snippet == marker.Snippet);
        }

        Pin LookupNonCllusteredPin(Marker marker)
        {
            return GetNonClusteredItems(Map).FirstOrDefault(outerItem => ReferenceEquals(outerItem.NativeObject, marker));
        }
        ...
        bool HandleGMSTappedMarker(MapView mapView, Marker marker)
        {
            ...
            if (targetPin == null)
                targetPin = LookupNonCllusteredPin(marker);
